...
mod test_module {
    ...
    use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, ResolveRecordResponse, InverseResolveRecordResponse};
    ...
    fn assert_inverse_resolve(deps: Deps, name: &str, owner: Addr) {
        let res = query(
            deps,
            mock_env(),
            QueryMsg::InverseResolve {
                addr: owner,
            },
        )
        .unwrap();

        let value: InverseResolveRecordResponse = from_binary(&res).unwrap();
    ...
    fn mock_owner_updates_config(deps: DepsMut) {
        // alice can register an available name
        ...
        let msg = ExecuteMsg::UpdateConfig { config: new_config };
        let _res = execute(deps, mock_env(), info, msg)
            .expect("contract successfully handles Register message");
    }
    ...
    #[test]
    fn proper_config_update() {
        let mut deps = mock_dependencies();

        mock_init_no_price(deps.as_mut());
        mock_owner_updates_config(deps.as_mut());
    ...
    fn register_available_name_and_query_works() {
        ...
        assert_inverse_resolve(deps.as_ref(), "alice", Addr::unchecked("alice_key"));
    }
    ...
    fn register_available_name_and_query_works_with_fees() {
        ...
        assert_inverse_resolve(deps.as_ref(), "bob", Addr::unchecked("bob_key"));
    }
    ...
    fn transfer_works() {
        ...
        assert_inverse_resolve(deps.as_ref(), "alice", Addr::unchecked("bob_key"));
    }
